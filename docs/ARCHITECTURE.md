...
**记忆插件都在比「记得多」，它只比「查得省」。** 这句话落到架构上，就是「四段流水线 + 三条不变量」。

**四段流水线**：日志进、素材出；素材进、提炼出；提炼进、分层库出；查询只取命中片段。
...
**三条不变量**（破了任何一条，整套设计就不成立）：

| # | 不变量 | 含义 |
|---|---|---|
| 1 | **零常驻** | 记忆不进系统提示；要查才查，只回命中片段 |
| 2 | **零依赖** | 只用 Node 内置模块：无数据库、无服务端、无后台进程 |
| 3 | **可溯源** | 每条记忆指得回哪次会话的哪句话，`verify` 能拉出原文 |
